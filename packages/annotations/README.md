...
Config also resolves from the `<script>` tag's `data-*` attributes (`data-sheet`, `data-snapshot`, `data-attribute`, `data-poll`, `data-start-visible`, `data-observe`, `data-debug`) or a `window.AnnotationsConfig` global. Precedence: `init()` argument › `window.AnnotationsConfig` › script `data-*`.
